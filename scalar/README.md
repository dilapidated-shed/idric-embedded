...
Target: classic AVR as represented by an ATmega328P assembly check. The kernel
deliberately avoids depending on hardware multiply for `Bits8`; multiplication
is an eight-step shift/add loop. Eight-bit arguments/results use the avr-gcc
scalar convention (`r24`, second operand `r22`); raw 32-bit binary32 payloads
use `r22:r23:r24:r25` from least to most significant byte.

The assembly check pins `-mmcu=atmega328p`; that proves this concrete encoding
profile, not every historical ATmega member in the family-wide inventory.
