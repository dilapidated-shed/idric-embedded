...
Target: ESP32-C3-class RV32IMC scalar CPU path. The implementation uses only
RV32I plus `M` for `Bits8` multiplication and explicitly disables compressed
instruction emission in this handwritten source. The same kernel is structurally
usable on ESP RISC-V profiles that contain those operations; that is not a claim
about the separate Xtensa targets. Standard RISC-V scalar input/result is `a0`;
a second scalar is `a1`.

`check.sh` assembles with `clang --target=riscv32-none-elf -march=rv32imc -mabi=ilp32`.
