...
Target: base MSP430 instruction surface. The kernel assumes neither the hardware
multiplier nor MSP430X-only instructions. Eight-bit input/result uses the low
byte of `r12`, with the second input in `r13`; a raw 32-bit binary32 payload uses
`r12` as the low word and `r13` as the high word.

The source has been reviewed against the branch's base-MSP430 instruction
inventory. It still requires an MSP430 assembler or target toolchain run before
assembly acceptance can be claimed. `check.sh` fails explicitly when
`msp430-elf-as` is unavailable.
